...
However, if it's a brand-new feature that's being tested or is recently added to master, you'll need to install the new version of `oref0` first.  By the way, if you want to check which version of oref0 you are currently running, `npm list -g oref0` ( this doenst work on Pizero w hat ) and if you want to check which branch `cd ~/src/oref0` and then `git branch`. 
